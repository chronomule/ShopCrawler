...
class letsbuy(CrawlSpider):
	...
	def parse(self, response):
		...
		try:
			#name: String
			specscount=1
			specs=hxs.select("//div[@class='prod_spec']")
			for spe in specs:
				scount=1
				if hxs.select("//div[@class='prod_spec']/table["+str(specscount)+"]/td[1]").extract()[0].strip()=="General Form":
					generalform=re.sub('[\D]',' ',hxs.select("//div[@class='prod_spec']/table["+str(scount)+"]/tr[2]/td[2]").extract()[0].strip())
					item['General Form'] = hxs.select("//div[@class='prod_spec']/table[@class='specification_table']//tr[2]/td[2]/text()").extract()[0]
		except:
			item['specs'] = []
		item['shippingCost'] = 0
		item['upc']=0
