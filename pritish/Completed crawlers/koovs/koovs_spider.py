...
class Koovs(CrawlSpider):
	...
	def parse(self, response):
		...
		try:
			#name: StringList
			categories = hxs.select("//div[@class='breadcrumbBlk']/ul/li//text()").extract()
			cat=[]
			for category in categories:
				if category.strip()!='Home' and category.strip()!= '' and category.strip()!='&gt;' and category.strip()!='>' and category.strip()!='home':
